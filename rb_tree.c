...
int
rb_tree_node_cmp_ptr_cb (struct rb_tree *self, struct rb_node *a, struct rb_node *b, unsigned char mode) {
...
rb_tree_test (struct rb_tree *self, struct rb_node *root) {
    ...
    if ( root == NULL )
        ...
        if ( ( ln != NULL && self->cmp(self, ln, root, 1) >= 0 )
            || ( rn != NULL && self->cmp(self, rn, root, 1) <= 0))
        {
...
rb_tree_find(struct rb_tree *self, void *value) {
    ...
    if (self) {
        ...
        int cmp = 0;
        while (it) {
            if ((cmp = self->cmp(self, it, &node, 1))) {
    ...
    if (self && node) {
        if (self->root == NULL) {
            ...
            while (1) {
                ...
                if (self->cmp(self, q, node, 0) == 0) {
                    break;
                }

                last = dir;
                dir = self->cmp(self, q, node, 0) < 0;
...
rb_tree_remove_with_cb (struct rb_tree *self, void *value, rb_tree_node_f node_cb) {
    if (self->root != NULL) {
        ...
        while (q->link[dir] != NULL) {
            ...
            g = p, p = q;
            q = q->link[dir];
            dir = self->cmp(self, q, &node, 1) < 0;
      
            // Save the node with matching value and keep
            // going; we'll do removal tasks at the end
            if (self->cmp(self, q, &node, 1) == 0) {
                f = q;
            }
